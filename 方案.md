...
创建群组时，认证中心（控制平面 / Local IdP）除维护文档树与逐节点 ACL 外，还需维护以下三类**群组级**元数据：

| 元数据 | 说明 |
|--------|------|
| **1. 角色列表** | 群组内可用的命名角色集合（如「管理员」「协作者」「读者」及后续自定义角色） |
| **2. 角色与用户映射** | 将成员 `node_id` 绑定到一个或多个群组角色（用于批量授权与默认策略计算） |
| **3. 默认角色规则** | 入群默认值 + 新建节点时的 ACL 模板（见下） |

**初始角色列表（三种内置角色）：**

| 角色 | 说明 |
|------|------|
| 管理员 | 群组内高权限协作角色；**不等于**「群主」身份本身 |
| 协作者 | 可读写文档内容、参与协作 |
| 读者 | 只读参与 |

**管理权限（仅群主）：**

* **群主** = 创建该群组的节点（`owner_node_id`），拥有群组治理权。
* **仅群主**可修改：角色列表、角色与用户映射、默认角色规则。
* 其他成员即使在某文档上具备 Operable（级别 3），也不得擅自改群组级 RBAC 配置（由控制平面 RPC 鉴权保证）。

**默认角色规则包含两条可配置项：**

1. **新成员默认角色**  
   * 规定：用户通过邀请**新加入**群组时，自动获得的群组角色。  
   * **初始值：** 读者。  
   * 群主可修改，例如改为「新加入成员一律为协作者」。

2. **新建目录/文档的 ACL 模板**  
   * 规定：调用 `CreateChild`（或等价 RPC）**新建**目录或文档节点时，按角色为**所有**角色批量写入该 `doc_id` 上的权限级别 $p \in \{0,1,2,3\}$（见 §2.3）。  
   * **初始模板：**
   ...
   * 群主可修改该模板；也可在角色列表中**新增角色**（如「审查员」），并在模板中指定其对新建节点的级别（例如审查员 = 1）。  
   * 对**已有**节点的 ACL，仍通过 `SetACL` 按用户/节点单独调整，不受「子权限不得高于父权限」一类约束。

**与逐节点 ACL 的关系：**

* 每个 `doc_id` 仍保存独立的 `(node_id → role)` 授权记录（或等价结构）；JWT 签发时以**该节点**上的有效级别为准。
* **不再要求** $p_u(\text{child}) \le p_u(\text{parent})$：父目录可读/可写，不自动限制子文档可设为更高或更低级别；子节点是否可见、可解密，仅由该子节点上的 ACL 与 RBAC 展开结果决定。
* 「父节点可读不自动等价于子节点可读」仍然成立（见 §2.4）：父级 JWT 不会隐式下发子级 `Key(doc)`。

**实现分期说明：** P2 起可先以「群主 + 逐节点 ACL」落地；角色列表 / 映射 / 默认规则表结构在 P2+ 逐步入库，`CreateChild` 时按默认模板批量写 ACL，`AcceptInvitation` 时按默认角色绑定成员。
...
* **父节点可读不自动等价于子节点可读**：子节点 `doc_id` 单独存 ACL（可由 §2.2 默认模板初始化，也可被 `SetACL` 覆盖）；仅当 $p_u(x) \ge 1$ 时，认证中心才在 JWT 中下发该节点的 `Key(x)`。
...
- [ ] 群组 RBAC：角色列表、角色-用户映射、默认角色规则（仅群主可改）
